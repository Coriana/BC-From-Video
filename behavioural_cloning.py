...
KL_LOSS_WEIGHT = 1.0 if USING_FULL_DATASET else 1.0
MAX_GRAD_NORM = 5.0
...
def behavioural_cloning_train(data_dir, in_model, in_weights, out_weights):
    ...
    for batch_i, (batch_images, batch_actions, batch_episode_id) in enumerate(data_loader):
        ...
        loss_sum += batch_loss
        batchnum = batch_i+1
        if batchnum % LOSS_REPORT_RATE == 0:
            time_since_start = time.time() - start_time
            print(f"Time: {time_since_start:.2f}, Batches: {batchnum}, Avrg loss: {loss_sum / LOSS_REPORT_RATE:.4f} Time per frame:{time_since_start/batchnum/BATCH_SIZE}")
            loss_sum = 0
            
        if batchnum % SAVE_BATCHES == 0:
            now = datetime.datetime.now()
            now.strftime('%Y%m%d%H%M%S')
            tmp_out_path = out_weights[:-8]
            tmp_out_path = tmp_out_path + now.strftime('_%Y%m%d_%H%M%S') + ".weights"
            state_dict = policy.state_dict()
            th.save(state_dict, tmp_out_path)
            time_since_start = time.time() - start_time
            print(f"Time: {time_since_start:.2f}, Batches: {batchnum}, Saved Current state to: {tmp_out_path}")

        if batchnum % BCKUP_BATCHES == 0:
            state_dict = policy.state_dict()
            th.save(state_dict, bkup_out_path)
            print("Backed up.")

    state_dict = policy.state_dict()
    th.save(state_dict, out_weights)
    time_since_start = time.time() - start_time
    print(f"Time: {time_since_start:.2f}, Batches: {batchnum}, Saved To: {out_weights}")
